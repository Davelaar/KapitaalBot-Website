...
function summarizeEdgeboardSignals(
  ...
  return rows
    ...
    .map((r) => {
      ...
      const scaledGross =
        typeof r.expected_gross_move_bps === "number" ? r.expected_gross_move_bps.toFixed(1) : null;
      const statisticalGross =
        typeof r.statistical_gross_bps === "number" ? r.statistical_gross_bps.toFixed(1) : null;
      const scale =
        typeof r.pooled_semantic_gross_scale === "number" ? r.pooled_semantic_gross_scale.toFixed(3) : null;
      const grossPart =
        scaledGross || statisticalGross
          ? `, gross(stat→scaled) ${statisticalGross ?? "—"}→${scaledGross ?? "—"} bps${
              scale ? `, scale ${scale}` : ""
            }`
          : "";
      return `${r.symbol} (#${r.rank}, ${r.route_name}, edge ${edge} bps, conf ${conf}, boost ${boost}${grossPart})`;
    })
    .join(" · ");
}
