...
export default function RecentExecutionSection({
  ...
  const orders = (trading.recent_orders ?? []).slice(0, maxOrders);
  const fills = (trading.recent_fills ?? []).slice(0, maxFills);
  const topRejectReasons = trading.top_reject_reasons_last_hour ?? [];
  if (orders.length === 0 && fills.length === 0 && topRejectReasons.length === 0) return null;
    ...
    <section style={{ marginTop: "2rem" }}>
      ...
      {topRejectReasons.length > 0 && (
        <div className="card" style={{ padding: "1rem 1.25rem", marginBottom: "1.25rem" }}>
          <h2 style={{ fontSize: "1rem", marginBottom: "0.5rem" }}>Top reject reasons (1h)</h2>
          <p style={{ margin: 0, fontSize: "0.875rem", color: "var(--muted)", lineHeight: 1.6 }}>
            {topRejectReasons.map((row) => `${row.label} (${row.count})`).join(" · ")}
          </p>
        </div>
      )}
            ...
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              <thead>
                <tr>
                  ...
                  <th style={th}>Execution bucket</th>
