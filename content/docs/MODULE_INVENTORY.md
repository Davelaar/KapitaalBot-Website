...
- **Primair:** TP/SL first-touch resolver op `trade_samples` voor de paper margin-lane.
...
- **Secundair:** Voedt store vanuit research-aligned signals.
...
- **~220 modules** onder `src/`, gegroepeerd in **~24 directories** (exclusief puur `mod.rs` re-export stubs die geen eigen logica bevatten — die staan wel in de boom maar zijn hier als “module root” beschreven waar relevant).
...
*Document gegenereerd voor technische documentatie. Werk bij bij grote architectuurwijzigingen (nieuwe directories, nieuwe env gates, hernoemde SSOT tabellen).*
