...
- **Website commit** — `f7239fd` (*docs: sync engine doc set 01-08, catalog + i18n, refresh sync scripts*).
- **Deploy server** — `cd /srv/KapitaalBot-Website && git pull --ff-only && npm ci && npm run build && systemctl restart kapitaalbot-web.service`.
...
- **Deploy server** — `/srv/krakenbot`: `git pull --ff-only`, `cargo build --release`, observability-export (timer of `systemctl start --no-block krakenbot-observability-export.service` — `start` zonder `--no-block` kan lang blokkeren tot de oneshot klaar is). `/srv/KapitaalBot-Website`: `git pull --ff-only`, `npm ci` of `npm install`, `npm run build`, `systemctl restart kapitaalbot-web.service`. **Let op:** `styled-jsx` staat expliciet in `package.json` zodat `next build` op de server `styled-jsx/package.json` resolveert (npm-hoisting). Live: `curl -sfL -o /dev/null -w '%{http_code}\n' https://kapitaalbot.nl/nl/dashboard` → 200.
